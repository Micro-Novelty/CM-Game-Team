...
class AgentDistributedInference:
    ...
    def connect_to_agent(self, host, port):
        self.established_connection = self.query_node._establish_peer_nodes(self.temporary_agent_id)
        ...
        try:
            ...
            if self.established_connection:
                ...
                if not info or not self._authenticate_agent(info.get('token', ''), agent_id):
                    print(f"[-] Authentication failed for agent with address: {host}:{port}")
    ...
    def _handle_client(self, client, addr):
        ...
        while self.running:
            try:
                ...
                if not message or not self._authenticate_agent(message.get('token', ''), agent_id):
                    print(f"[-] Authentication failed for agent with address: {addr}")
                    self._log_security_event('authentication_failed', {'agent': agent_id})
                    self.report_failure(id(self), 'authentication', reason=f'Failed authentication from {addr}')
                    break
