...
class AgentDistributedInference:
    ...
    def _handle_predict_request(self, message, sender_id, method='basic_prediction'):
        ...
        if method == 'basic_prediction' and self.predict_manager is None:
            ...
            try:
                result, chosen_label, confidence = self.predict_manager.advanced_prediction_method(titles, label_map, rules, show_proba=True, use_transformer=True)
...
class PipelineAsyncManager:
    ...
    def __init__(self, pipeline, prediction_manager, config: SecurityConfig=None, security_level: SecurityLevel = SecurityLevel.STAGING, state_file: str = None, max_workers=4, task_timeout=30, max_retries=3):
        self.pipeline = pipeline
        ...
        if self.config.require_api_key:
            self._default_api_key = self.api_key_manager.generate_key({'type': 'default'})
    ...
    def predict(self, texts, timeout: float = None, retries: int = None, api_key: str = None, client_ip: str = None, method: str = 'advanced') -> Dict[str, Any]:
        """
    ...
    def _predict_sync(self, text: str, timeout: float) -> Dict[str, Any]:
    ...
    def _advanced_predict_sync(self, test_titles, label_map, rules, agent_id: str, use_transformer: bool=False, timeout: float = 30.0) -> Dict[str, Any]:
        # Internal synchronous prediction.
    ...
    async def _predict_with_timeout(self, text: str, timeout: float) -> Dict[str, Any]:
    ...
    def advanced_batch_prediction(self, test_titles, label_map, rules):
        try:
            ...
            if not self.pipeline.intents:
                advanced_result = self.predict(text, timeout=30) # regular predict to populate pipelines data
            else:
    ...
    def predict_batch(self, texts: List[str], timeout: float = None) -> List[Dict[str, Any]]:
        """
        ...
        results = []
        for text in texts:
            result = self.predict(text, timeout)
            results.append(result)
        return results
...
class CohesiveAgentDeployment:
    ...
    def _is_server_listening(self) -> bool:
        # check if the server is actually listening on its port
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.settimeout(1)
        try:
...
async def run_secure_agent_cluster(test_titles, label_map, rules, agent_id, filename, title_name, label_name, manager):
    ...
    try:
        ...
        success1 = await agent1._connect_with_smart_retry(agent1, "127.0.0.1", 5556, api_key)
        if success1:
...
async def example_async_with_result_queue(test_titles, label_map, rules, agent_id, filename, title_name, label_name):
    ...
    start_time = time.time()
    results = await agent.predict_batch_async(texts, api_key=api_key)
    elapsed = time.time() - start_time
...
def initiate_prediction_usage(pipeline, manager, predict_wrapper, test_titles, label_map, rules):
    ...
    with predict_wrapper as wrapper:
        ...
        regular_predict = wrapper.predict(texts, timeout=60)

        print('[==] Initiating advanced batch prediction')
        predicted_output = wrapper.advanced_batch_prediction(test_titles, label_map, rules)
...
def initiate_with_retries(pipeline, manager, wrapper, test_titles, label_map, rules):
    ...
    try:
        ...
        texts = {"test_titles": test_titles, "label_map": label_map, "rules": rules, "use_transformer": True}
        result = wrapper.predict(texts, timeout=60, retries=3)
