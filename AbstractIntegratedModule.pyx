    ...
    def _calibrate_probs(self, probs, target_preds, attn_weights, input_ids):
        ...
        for i in range(batch_size):
            ...
            if attn_weights is not None and i < len(attn_weights):
                ...
                attn_quality = np.std(attn) if attn.size > 0 else self.pipeline.confidence_threshold
                abstract_score = self.pipeline.confidence_threshold + attn_quality * anisotropy
