...
class GeometricWeightShaping:
    ...
    def eigenvalue_encoder(self, x):
        ...
        if X.ndim > 1 and X.shape[1]:
            X = X.ravel()[:X.size - (X.size % 2)].reshape(-1, 2)    
...
class IntegratedPipeline:
    def __init__(self, memory_name='agent_memory', use_async=False, agent_port=None, ssl_cert_file=None, ssl_key_file=None, secret_key=None, shared_auth_token=None, predict_manager=None):
        ...
        self.port = agent_port or int(os.environ.get('AGENT_PORT', 5555))
    ...
    def lstm_setup_inference(self, raw_X, raw_Y, n_display=3):
        print("\n" + "=" * 55)
...
class CohesiveAgentDeployment:
    ...
    def __init__(self,
        ...
        if not self.pipeline.distribution._server_started:
